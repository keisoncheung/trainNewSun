...
namespace trainNewSun
{
    public partial class FrmTeacher : Form
    {
        ...
        public FrmTeacher(FrmMain frmParent)
        {
            InitializeComponent();
        ...
        public FrmTeacher(string itemno)
        {
            InitializeComponent();
            strItemNO = itemno;
            loadData();
            ...
            if (hea != null)
            {
                ...
                if (hea.HaveClass_Detail != null && hea.HaveClass_Detail.Count>0)
                {
                    foreach (tblHaveClass_Detail det in hea.HaveClass_Detail)
                    {
                        ...
                        dgvData.Rows[dgvData.Rows.Count - 1].Cells["personCount"].Value = det.personCount;
        ...
        public void setTotalQty()
        {
            ...
            {
                persons += Convert.ToInt32(Convert.ToString(dgvData.Rows[i].Cells["personCount"].Value));
            ...
            labPersonCount.Text = Convert.ToString(persons);
        ...
        private void btCollections_Click(object sender, EventArgs e)
        {
            ...
            decimal _AmountTotal = Convert.ToDecimal(labAmountTotal.Text.Trim());

            if (string.IsNullOrWhiteSpace(money) || money == "0")
            {
                MessageBox.Show("对不起，本次支付金额填写处为零，暂时不能支付！");
                return;
            }
            if (Convert.ToDecimal(money) > _AmountTotal)
            {
                MessageBox.Show("对不起，本次支付金额填写不能大于应付总额，请重新修改支付金额！");
                tbxPayMonent.Focus();
                return;
            }
            ...
            {
                MessageBox.Show("对不起，目前还没有进行过任何课程，暂时不能支付！");
                return;
            }

            

            if (MessageBox.Show("支付金额前请确实金额额度和支付时间是否正确，是否继续?", "支付金额", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                if (bhch.PayMoney(hea))
                {
                    ...
                    labAmountTotal.Text = Convert.ToString(Math.Round(_AmountTotal-Convert.ToDecimal(money),2));
                    paym();

                }
                else
                    MessageBox.Show("支付失败，请稍候再试！");
           }
        }
        ...
        public void save(string str)
        {
            ...
            if (dgvData.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < CountRows; ++i)
                {
                    ...
                    _hcd.cid = hea.cid;
                    _hcd.personCount = Convert.ToString(dgvData.Rows[i].Cells["personCount"].Value);
        ...
        //{
        //    FrmmenberQ frm = new FrmmenberQ(true,"T");
        //    frm.ShowDialog();
        //    if (!string.IsNullOrEmpty(frm.result))
        //    {
        //        get_menber();
        ...
        private void dgvData_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (dgvData.Rows.Count > 0)
            {
                ...
                {
                    string persons = Convert.ToString(dgvData.Rows[e.RowIndex].Cells["personCount"].Value);
                    string amounts = Convert.ToString(dgvData.Rows[e.RowIndex].Cells["amount"].Value);
                    if (string.IsNullOrEmpty(persons))
                    {
                        dgvData.Rows[e.RowIndex].Cells["personCount"].Value = "0";
                    }
